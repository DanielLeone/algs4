...
public class Point implements Comparable<Point> {
    ...
    private class PointComparator implements Comparator<Point> {
        ...
        public int compare(Point pointA, Point pointB) {
            ...
            // the point (x1, y1) is less than the point (x2, y2) if and only if the slope (y1 − y0) / (x1 − x0) is less than the slope (y2 − y0) / (x2 − x0)
            double slopeFromA = Point.this.slopeTo(pointA);
            double slopeFromB = Point.this.slopeTo(pointB);
            return Double.compare(slopeFromA, slopeFromB);
        }
    }
}